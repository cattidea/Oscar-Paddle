...
class RetrievalDataset(Dataset):
    ...
    def __init__(self, split, cfg, tokenizer, training=True):# {{{
        ...
        # Self.feat_dir is a dictionary,
        # which contains image_id.npy file
        self.feat_dir = os.path.join(self.data_dir, '{}_img_frcnn_feats'.format(split))
        ...
        assert len(os.listdir(self.feat_dir)) == len(self.captions), \
            "The length of image features and captions dose not match!"

        if cfg['INPUT']['ADD_OD_LABEL']:
            od_label_file = os.path.join(self.data_dir, '{}_vg_labels.pd'.format(split))
            self.labels = paddle.load(od_label_file)
    ...
    def get_feat(self, image_id):
        feat = np.load(os.path.join(self.feat_dir, str(image_id) + '.npy'))
        return paddle.to_tensor(feat)
